...
class TestFormatRef:
    ...
    def test_format_with_default_provider(self):
        ...
        result = format_ref("workx", "task", "task_01JABC")
        assert result == "workx:task:local:task_01JABC"
...
class TestParseRef:
    ...
    def test_parse_4_segment(self):
        ...
        ref = parse_ref("workx:task:local:task_01JABC")
    ...
    def test_parse_3_segment_legacy(self):
        ...
        ref = parse_ref("workx:task:task_01JABC")
        assert ref.domain == "workx"
        assert ref.entity_type == "task"
...
class TestTypedRef:
    ...
    def test_str_representation(self):
        ...
        ref = TypedRef("workx", "task", "local", "task_01JABC")
        assert str(ref) == "workx:task:local:task_01JABC"
...
class TestValidateRef:
    ...
    def test_validate_valid_4_segment(self):
        ...
        is_valid, error = validate_ref("workx:task:local:task_01JABC")
        assert is_valid is True
        assert error is None

    def test_validate_valid_3_segment(self):
        """Valid 3-segment legacy ref passes validation."""
        is_valid, error = validate_ref("workx:task:task_01JABC")
        assert is_valid is True
        assert error is None
    ...
    def test_is_valid_true(self):
        ...
        assert is_valid_ref("workx:task:local:task_01JABC") is True
...
class TestConvenienceFunctions:
    ...
    def test_workx_ref(self):
        """workx_ref creates correct reference."""
        result = workx_ref("task", "task_01JABC")
        assert result == "workx:task:local:task_01JABC"

    def test_workx_ref_with_provider(self):
        """workx_ref with explicit provider."""
        result = workx_ref("task", "task_01JABC", "remote")
        assert result == "workx:task:remote:task_01JABC"
...
class TestIsKnownDomain:
    ...
    def test_known_domain_workx(self):
        """workx is a known domain."""
        assert is_known_domain("workx") is True
...
class TestIntegrationScenarios:
    ...
    def test_workx_task_ref(self):
        """Workx task reference example."""
        ref = workx_ref("task", "task_01JXYZ")
        assert ref == "workx:task:local:task_01JXYZ"

    def test_workx_decision_ref(self):
        """Workx decision reference example."""
        ref = workx_ref("decision", "dec_01JXYZ")
        assert ref == "workx:decision:local:dec_01JXYZ"
    ...
    def test_legacy_ref_migration(self):
        ...
        legacy = "workx:task:task_old_id"
        canonical = canonicalize_ref(legacy)
        assert canonical == "workx:task:local:task_old_id"
