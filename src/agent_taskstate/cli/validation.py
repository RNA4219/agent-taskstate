"""CLI validation and typed-ref guards."""
...
def _normalize_optional_json_text(value: Any) -> Optional[str]:
    if value is None:
        return None
    return value if isinstance(value, str) else jdump(value)


def canonicalize_refs(values: Any, field: str) -> list[str]:
    if not isinstance(values, list):
        raise AgentTaskstateError(f"{field} must be array")
    result: list[str] = []
    for value in values:
        if not isinstance(value, str):
            raise AgentTaskstateError(f"{field} must contain strings")
        try:
            result.append(canonicalize_ref(value))
        except ValueError as exc:
            raise AgentTaskstateError(f"invalid {field} ref: {exc}") from exc
    return result


def canonicalize_optional_ref(value: Any, field: str) -> Optional[str]:
    if value is None:
        return None
    if not isinstance(value, str):
        raise AgentTaskstateError(f"{field} must be a string")
    try:
        return canonicalize_ref(value)
    except ValueError as exc:
        raise AgentTaskstateError(f"invalid {field}: {exc}") from exc
...
def validate_task_payload(data: Dict[str, Any]) -> None:
    ...
    if data.get("retry_count") is not None and (
        not isinstance(data["retry_count"], int) or data["retry_count"] < 0
    ):
...
def validate_state_payload(data: Dict[str, Any]) -> None:
    ...
    for field in ("constraints", "done_when", "artifact_refs", "evidence_refs"):
        if not isinstance(data.get(field, []), list):
            raise AgentTaskstateError(f"{field} must be array")
    data["artifact_refs"] = canonicalize_refs(data.get("artifact_refs", []), "artifact_refs")
    data["evidence_refs"] = canonicalize_refs(data.get("evidence_refs", []), "evidence_refs")
...
def is_done_when_satisfied(done_when: List[Any]) -> bool:
    return bool(done_when) and all(
        isinstance(item, dict) and bool(item.get("done", False)) for item in done_when
    )
...
def validate_status_transition(
    ...
    if to_status == "ready":
        ...
        if not state or not state["done_when"]:
            raise DependencyBlockedError(
                "done_when must contain at least one item before moving to ready"
            )
    ...
    if to_status == "review":
        if count_open_high_questions(conn, task_row["id"]) > 0:
            raise DependencyBlockedError(
                "high priority open questions must be 0 before moving to review"
            )
        if count_relevant_decisions(conn, task_row["id"]) == 0:
            raise DependencyBlockedError(
                "at least one accepted/proposed decision is required before moving to review"
            )
    if to_status == "done":
        if from_status != "review":
            raise InvalidTransitionError("done is only allowed from review")
        if not state or not is_done_when_satisfied(state["done_when"]):
            raise DependencyBlockedError(
                "all done_when items must be satisfied before moving to done"
            )
