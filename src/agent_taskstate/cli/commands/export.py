    ...
    with connect(ctx.db_path) as conn:
        ...
        decisions = [
            ...
            for r in conn.execute(
                "SELECT * FROM decisions WHERE task_id = ? ORDER BY created_at ASC", (args.task,)
            ).fetchall()
        ]
        questions = [
            row_to_question(r)
            for r in conn.execute(
                "SELECT * FROM open_questions WHERE task_id = ? ORDER BY created_at ASC",
                (args.task,),
            ).fetchall()
        ]
        runs = [
            row_to_run(r)
            for r in conn.execute(
                "SELECT * FROM runs WHERE task_id = ? ORDER BY created_at ASC", (args.task,)
            ).fetchall()
        ]
        bundles = [
            row_to_bundle(r)
            for r in conn.execute(
                "SELECT * FROM context_bundles WHERE task_id = ? ORDER BY created_at ASC",
                (args.task,),
            ).fetchall()
        ]
    export = {
