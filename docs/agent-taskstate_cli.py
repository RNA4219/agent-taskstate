...
def typed_ref(
    domain: str, entity_type: str, entity_id: str, provider: str = "local"
) -> str:
    """Generate 4-segment typed_ref: <domain>:<entity_type>:<provider>:<entity_id>"""
...
def validate_task_payload(data: Dict[str, Any]) -> None:
    ...
    require_in(data.get("status", "proposed"), TASK_STATUSES, "status")
...
def cmd_task_set_status(ctx: AppContext, args: argparse.Namespace) -> int:
    ...
    if args.to in {"cancelled", "in_progress"} and args.reason_required and not args.reason:
